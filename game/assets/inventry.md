...
- animations: なし。
- 水源メッシュは含まれていない → `water_source` は primitive `CylinderMesh` 維持。
...
- 適用箇所: `TerrainBuilder._add_ground_glb_backdrop()` が平面グラウンド生成直後に `PackedScene` として instantiate し、マップ中心へ配置。
...
**未取得**。所有している人型モデルは Unreal Engine 向けで Godot への変換が未完了。変換ができるまで `CitizenFactory` はチェスポーン風の primitive を継続使用。
...
- [ ] 資源・水: パックに水メッシュなし → primitive 継続
- [x] 地形: `terrian.glb` を装飾背景として `TerrainBuilder._add_ground_glb_backdrop()` で配置（scale/Y はエディタ実機で調整）
- [ ] 住民: Unreal → Godot 変換完了まで保留
