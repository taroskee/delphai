...
- [x] **検証 (3 点)**: `cargo test --workspace` 105 pass ✅ / `make build` ✅ / **人間目視 — Mac で `make build-mac` → Godot で world.tscn 起動 → 住民が滑らかに動く（マス目でない）を確認**
...
- [x] 検証 (3 点): `cargo test --workspace` 109 pass ✅ / `make build` ✅ / **人間目視 — Mac で `make build-mac` → Godot で world.tscn 起動 → 住民が連続移動 & マウスドラッグ/ホイール/WASD/トラックパッドで pan/zoom 動作を確認**
