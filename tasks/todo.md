...
- [ ] コミット: `chore(rebuild): strip Phase 1 code down to LLM + citizen type`（ユーザー指示待ち）

### Sprint N1: Rust 最小 World — tick + citizen spawn（TDD）

- [ ] RED: `crates/delphai-core/src/world.rs` 新規、`World::new()` + `tick()` で `tick_count` が 1 増える失敗テスト
- [ ] GREEN: `World { tick_count: u32, citizens: Vec<Citizen> }` と `tick()`
- [ ] RED: `spawn_citizen(name, tile_pos)` テスト
- [ ] GREEN: `TilePos { x: i16, y: i16 }`（`pathfinding.rs` 新規）と `spawn_citizen`
- [ ] `agent/citizen.rs` を新 `World` と整合（LLM 依存のシグネチャは維持）
- [ ] 検証: `cargo test -p delphai-core` / `cargo clippy --workspace`

### Sprint N2: 位置と 1-step 移動 + サブ tick 補間（最初から組み込む）

- [ ] RED: `MoveState { tile_pos, prev_tile_pos, move_target }`、tick で target に向かい 1-step、`World::get_citizen_world_pos(idx, alpha)` が prev→curr を線形補間する失敗テスト
- [ ] GREEN: 最小実装（履歴配列・cooldown は **導入しない**、止まって見える問題を最初から避ける）
- [ ] RED: 16 tick 連続で静止 tick が 2 連続を超えないテスト（R5.4 Phase C.1 の regression guard）
- [ ] GREEN: 最小ロジック
- [ ] 検証: `cargo test -p delphai-core` / `cargo clippy --workspace`

### Sprint N3: gdext FFI 最小再構築 — WorldNode

- [ ] `crates/delphai-gdext/src/lib.rs` に `WorldNode` 再追加
- [ ] FFI: `initialize()` / `tick()` / `get_citizen_count()` / `get_citizen_name(i)` / `get_citizen_world_pos(i, alpha) -> Vector2`
- [ ] gdext 単体テスト（cargo test）
- [ ] 検証: `cargo test --workspace` / `make build` 成功
