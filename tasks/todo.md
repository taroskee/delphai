...
- [ ] `crates/delphai-core/src/resource.rs` 新規
...
- [ ] `crates/delphai-core/src/lib.rs` に `pub mod resource;` 追加
- [ ] `crates/delphai-core/src/agent/behavior.rs` 全面改修
...
- [ ] `crates/delphai-core/src/world.rs` 大幅改修
...
- [ ] `crates/delphai-core/src/agent/conversation.rs` テスト修正
  - `BehaviorState::Sleeping` → `BehaviorState::Idle` に置換
  - `BehaviorState::Eating` → `BehaviorState::Gathering` に置換
- [ ] `crates/delphai-gdext/src/lib.rs` API整理
  - 削除: `build_conversation_prompt_str`, `record_heard_speech`, `apply_citizen_response`, `set_divine_voice`, `clear_divine_voice`, `grow_divine_awareness`, `get_divine_awareness`
...
- [ ] `game/scenes/world.tscn` 新規 — メインシーン（Node3D ルート）
- [ ] `game/scenes/world.gd` 新規 — `WorldNode` 呼び出し、tick駆動
- [ ] `game/scenes/citizen_body.tscn` 新規
  - `CharacterBody3D` + `MeshInstance3D`（カプセル仮置き）+ `Label3D`（名前表示）
- [ ] `game/scenes/citizen_body.gd` 新規
  - `walk_to(world_pos: Vector3)` — `NavigationAgent3D` または手動Tween移動
  - `set_behavior(state: String)` — アニメ切り替え（仮置きはMesh色変更）
- [ ] カメラ設定: `Camera3D` を俯瞰固定（0 A.D. スタイル、後でドラッグ対応）
- [ ] `game/scenes/terrain.tscn` 新規 — `StaticBody3D` + `MeshInstance3D`（平面グリッド）
- [ ] タイル座標 → 3D世界座標変換（`tile_to_world(TilePos) -> Vector3`）
- [ ] `world.gd` でtick毎に `get_citizen_tile_pos()` → `walk_to()` 呼び出し

### Sprint 3: 資源配置 + 採取ビジュアル

> 判定: 住民が berry bush に近づき、食べると量が減る

- [ ] `game/scenes/resource_node.tscn` — `StaticBody3D` + `MeshInstance3D`（色で種別区別）
- [ ] `world.gd` 起動時に `get_resource_count()` → 資源ノード生成
- [ ] tick毎に `get_citizen_behavior()` で状態取得 → Gathering中はアニメ（仮: scale pulse）
- [ ] 資源量に応じてMeshスケール変化（berry bushが減ると小さくなる）
- [ ] `get_citizen_fed()` / `get_citizen_hydration()` → デバッグUI（ProgressBar × n人）
...
- [ ] **Sprint 1完了後**: `cargo test` 全通過、needs-driven行動がテストで確認できる
- [ ] **Sprint 2完了後**: 3D空間で住民が歩き回る — 眺めて飽きないか？
