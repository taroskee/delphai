...
### Sprint 9: 3Dアセット適用 (Asset Integration)
...
- [ ] **[GDScript] 動物モデル差し替え** (`game/assets/animal/Animals_FREE.glb`)
  ...
  - アニメーション対応: `AnimationPlayer` ノードを取得して idle/run ステートを切り替え
  - Zip 展開済みの個別 GLB (`Separate_assets_glb.zip`) で個別アニメがあればそちら優先
...
- [ ] **[GDScript] 住民モデル** (humanoid primitive → GLB 候補確認)
  - `game/assets/animal/` に人型があれば差し替え。なければ primitive 継続
...
- [x] **Sprint 8**: 鹿が人間から逃げ、マップ端でフェードアウト、遠い位置へリスポーン (165テスト全通過)
- [ ] **Sprint 9**: GLBアセットを適用し、視覚的にリッチな世界になる
