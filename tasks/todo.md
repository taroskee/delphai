...
- [ ] スクリーンショット 2026-04-20: ゲーム画面の地形・市民が区別しづらい
  ...
  - 市民をTree_Stump_01にしているため、デフォルトのポーンに変更、将来はGLB 人型モデルに変更予定
...
#### 13.2 高度マップ → タイル分類（共通層）+ 視認性回復

> **key**: この分類ロジック＝水源・速度・鉱石配置の基盤。MVP/post-MVP で同じ。
> **視認性方針変更**: 当初は高度のみで全タイル種別を決める案だったが、川/森/山角は `_classify_layout` で決定論的に配置済み（村バランス維持）。レイアウト優先＋procgen 高所のみ T_MOUNTAIN へ昇格のハイブリッドに変更。視覚的な高度マップ可視化は Terrain3D auto-shader（grass↔dirt 自動ブレンド）で解決し、コントロールマップ手書きを回避。

- [x] `TerrainBuilder._classify_layout(col, row, map_w, map_h)` を旧 `get_terrain` から抽出（レイアウト専用）
- [x] `TerrainBuilder.classify_tiles_from_height(map_w, map_h, tile_size)` 実装:
  - `_classify_layout` で各タイル分類 → `T_GROUND`/`T_FOREST` のみ高度サンプリングで `MOUNTAIN_HEIGHT_THRESHOLD=3.0` 超を `T_MOUNTAIN` に昇格
  - 起動時 1 回計算 → `_tile_cache: PackedByteArray` に保存
- [x] `TerrainBuilder.get_terrain(col, row, map_w, map_h)` を O(1) キャッシュルックアップ化（キャッシュ未populated 時は `_classify_layout` フォールバック、API 不変）
- [x] `make_walkable_map()` はキャッシュ越しに自動利用（API 不変、内部のみ変更）
- [x] `world.gd::_ready` で `build_terrain3d` 直後に `classify_tiles_from_height` 呼び出し（features/walkable_map がキャッシュを参照する前）
- [x] **視認性対策**: `terrain_builder.gd::build_terrain3d` で auto_shader 有効化
  - `assets.set_texture(1, _create_dirt_asset())` で茶色 dirt をテクスチャ 1 に追加
  - `terrain.material.auto_shader = true`、`auto_slope = 10.0`、`blend_sharpness = 0.975`
  - 斜面が自動的に grass→dirt にブレンドされ、高度マップの尾根/谷が視認可能に
...
- [ ] Godot 実機起動 → SE 村を 5 分以上眺めていられるか（**Phase 1 判定**、ユーザ手動確認）
- [x] `cargo test -p delphai-core` 165 passed
- [x] `cargo clippy -p delphai-core -p delphai-gdext` 警告ゼロ
- [x] IDE 診断（`mcp__ide__getDiagnostics`）GDScript parser error なし
- [ ] コミット: `feat(terrain): tile classification cache + auto-shader heightmap reveal`
- [x] `game/assets/inventry.md` 更新済み（13.1 で terrian.glb → Terrain3D ProcGen 方針反映済み）
- [x] `tasks/lessons.md` に Terrain3D seed 固定の理由追記済み（13.1）
