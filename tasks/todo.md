...
- [ ] `crates/delphai-core/src/pathfinding.rs` 新規
...
- [ ] `crates/delphai-core/src/citizen.rs` に `MoveState` 追加
  - `tile_pos`, `move_target`, `move_history[16]`, `move_cooldown`, `stuck_counter`, `wander_center`, `wander_radius`
- [ ] `World::tick()` に移動処理追加（move_cooldown制御、wander目標選択）
- [ ] `WorldNode::set_walkable_map(data: PackedByteArray)` GDExt公開
- [ ] `WorldNode::get_citizen_tile_pos(idx) -> Vector2i` GDExt公開
- [ ] `WorldNode::get_citizen_facing(idx) -> int` GDExt公開
- [ ] `map.gd` に `get_walkable_data() -> PackedByteArray` 追加
- [ ] `main.gd` 起動時に `set_walkable_map()` 呼び出し
- [ ] chara スプライトシートの歩行フレーム数を実測確認（着手前必須）
- [ ] `citizen_sprite.gd` に `walk_to(screen_pos)` / `set_facing(dir)` 追加（Tween補間）
- [ ] `main.gd` でtick毎に tile_pos 変化を検知 → `sprite.walk_to()` 呼び出し
...
- [ ] crystalball ループアニメを声送信時に表示（`assets/effect/other/crystalballV001effect-loop/`）
